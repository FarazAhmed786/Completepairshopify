...
export default function TeamSingle({name,slug,id,updateTeams}) {
  ...
  return (
    <li className="divide-y">
        <div className="flex justify-between  items-center group hover:bg-neutral-50">
            <Link href={`/teams/${id}/?shop=${shop}&host=${host}`}>
                <a className="flex-grow text-sm truncate cursor-pointer" title="kk">
                    <div className="flex px-5 py-5">
                        <span>
                            <img alt="Team Logo" className="rounded-full h-auto w-9 rounded-full w-9 h-9 min-w-9 min-h-9" src={`https://eu.ui-avatars.com/api/?uppercase=false&name=${name}`}/>
                        </span>
